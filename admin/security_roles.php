...
//		$areas = sort_areas($areas);
		...
		$sections = array_values($sections);
     	...
     	if ($new_role) 
       	{
			add_security_role($_POST['name'], $_POST['description'], $sections, $areas,  $_POST['category_sales'], $_POST['curr_code']);
                        
			display_notification(_("New security role has been added."));
       	} else
       	{
			update_security_role($_POST['role'], $_POST['name'], $_POST['description'], 
				$sections, $areas,  $_POST['category_sales'], $_POST['curr_code']);
...
if (get_post('addupdateplugins'))
{
     if (get_security_role_plugins_by_rol_grp($_POST['role'],$_POST['group_sel'])==0){
        add_security_role_plugins($_POST['role'], $_POST['group_sel']);
     }
     else         
     display_error (_("El grupo ya existe"));
}
...
if (!isset($_POST['role']) || get_post('clone') || list_updated('role')) {
	...
	if ($id) {
		...
		$_POST['name'] = $row['role'];
                $category_sales = $row['sales_type_category'];
                $_POST['curr_code'] = $row['curr_code'];
//	if ($row['inactive']
//		$_POST['inactive'] = 1;
...
if ($Mode == 'Delete')
{			
    delete_security_role_plugins($selected_id);	
}
...
div_start('controls_prod');
if (isset($_POST['role']) && $_POST['role']!=0){

$_POST['category_sales']=$category_sales;
    
start_table(TABLESTYLE2);
table_section(1);
table_section_title(_("Filtros por Productos"));
category_sales_list_row(_("Category Sales").':','category_sales', null, false, false, false, _("Seleccione"));       
end_table(1);


$Ajax->activate('controls_prod');    
echo "<hr>";
...
$inactive = (isset($_POST['active']) && $_POST['active'] == 1 ? 1 : 0);
$result = get_security_role_plugins_all($_POST['role']);

while ($myrow = db_fetch($result))
{
    alt_table_row_color($k);
	label_cell($myrow["group_products"]);
	label_cell($myrow['description']);
	inactive_control_cell($myrow["id"], $myrow["inactive"], "security_roles_plugins", 'id');
...
table_section_title(_("Filtros por Productos"));
start_row();
    group_dimensions_list_cells(_("Groups:"), 'group_sel', null, true, _("Seleccione Groups Products"), false);
end_row();        
end_table(1);

submit_center_first('addupdateplugins', _("Add Role Plugins"), '', 'default');

echo "<hr>";

start_table(TABLESTYLE2);
table_section(1);
table_section_title(_("Otros Filtros"));
currencies_list_row(_("Customer's Currency:"), 'curr_code', null, false, true, 'Selec');
end_table(1);
