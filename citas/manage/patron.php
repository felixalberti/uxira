...
page(_("Maestro de patrones"), false, false, "", $js);
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
  ...
  if (strlen($_POST['patron']) == 0) 
	{
		$input_error = 1;
		display_error(_("Indique un c�digo para el patron."));
		set_focus('patron');
	}
	...
	if ($input_error !=1)
	{
    	if ($selected_id != -1) 
    	{
		  update_patron($selected_id, $_POST['descripcion']); 		
			display_notification(_('El item ha sido actualizado: '));
    	} 
    	else 
    	{
		    add_patron($_POST['patron'], $_POST['descripcion']);                         
...
function edit_tabla($row){
	  $link = edit_button_cell2("Edit".$row["patron"], _("Edit"));
		return $link;
}	
function borrar_tabla($row){
	  $link = delete_button_cell2("Delete".$row["patron"], _("Delete"));
		return $link;
}	
...
//------------------------------------------------------------------------------------------------
   $sql = "SELECT patron, descripcion FROM ".TB_PREF.
   "cm_patron";
 
$sql = $sql . " order by patron";
//display_notification($sql);
//------------------------------------------------------------------------------------------------

$cols = array(
	_("Patron") => array('align'=>'center'),
	_("Descripci�n") => array('align'=>'center'),
...
if ($selected_id != -1) {
    ...
    text_row(_("Patr�n:"), 'patron', null, 6, 6);    
}
    text_row(_("Descripci�n:"), 'descripcion', null, 20, 20);    
