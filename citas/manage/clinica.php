...
***********************************************************************/
//29/06/2010
$page_security = "SA_MEDICAL_APPOINMENT_CLINIQUE";
$path_to_root="../..";
...
include_once($path_to_root . "/citas/includes/db/clinica_db.inc");
...
page(_("Clinique"), false, false, "", $js);
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	$input_error = 0;
  if (strlen($_POST['coclini']) == 0) 
	{
		$input_error = 1;
		display_error(_("Indique un codigo para el coclini."));
		set_focus('coclini');
	}
	elseif (strlen($_POST['descripcion']) == 0) 
	{
		$input_error = 1;
		display_error(_("La descripcion no puede estar vacia."));
	...
	if ($input_error !=1)
	{
    	if ($selected_id != -1) 
    	{
		  update_clinica($selected_id, $_POST['descripcion']); 		
			display_notification(_('El item ha sido actualizado: '));
    	} 
    	else 
    	{
		    add_clinica($_POST['coclini'], $_POST['descripcion']);                         
...
function edit_tabla($row){
	  $link = edit_button_cell2("Edit".$row["codclini"], _("Edit"));
		return $link;
}	
function borrar_tabla($row){
	  $link = delete_button_cell2("Delete".$row["codclini"], _("Delete"));
		return $link;
}	
...
//------------------------------------------------------------------------------------------------
   $sql = "SELECT codclini, descripcion FROM ".TB_PREF.
   "cm_clinica";
 
$sql = $sql . " order by codclini";
...
if ($selected_id != -1) {
     label_row(_("Codigo:"), $_POST['coclini']); 
     hidden('coclini', $_POST['coclini']);
}
else {
    text_row(_("Codigo:"), 'coclini', null, 6, 6);    
}
