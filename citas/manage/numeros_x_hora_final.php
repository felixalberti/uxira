...
***********************************************************************/
$page_security = 'SA_MEDICAL_APPOINMENT_NUMBERXHOUR';
$path_to_root="../..";
...
page(_("Numeros/Hora"), false, false, "", $js);
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
  ...
  if (strlen($_POST['patron']) == 0) 
	{
		$input_error = 1;
		display_error(_("Seleccione un patr�n de la lista."));
		set_focus('patron');
	}
	elseif (strlen($_POST['tipo_cita']) == 0) 
	{
		$input_error = 1;
	...
	{
		$input_error = 1;
		display_error(_("El n�mero no puede estar vacio."));
		set_focus('numero');
	}
	...
	if ($input_error !=1)
	{
    	if ($selected_id != -1) 
    	{
		  update_item_numeros($selected_id, $_POST['patron'], $_POST['tipo_cita'], $_POST['numero'], $_POST['hora_cita'], $_POST['clinica'],
		  $_POST['codunidfunc']); 		
			//display_notification(_('El item ha sido actualizado: '));
    	} 
    	else 
    	{
		    add_item_numeros($_POST['patron'], $_POST['tipo_cita'], $_POST['numero'], $_POST['hora_cita'], $_POST['clinica'],
		  $_POST['codunidfunc']);                         
...
start_row();
    ...
    elseif (isset($_POST['patron'])){
     ...
     $tipocita = null;
	  ...
	  type_patron_list_row(_("Seleccionar un patr�n:"), 'selec_tabla', $codpatron, null, true);
...
/*function edit_tabla($row){
	  $link = edit_button_cell2("Edit".$row["patron"].';'.$row["tipo_cita"].';'.$row["numero"], _("Edit"));
		return $link;
}*/
function edit_tabla($row){
...
$cols = array(
	_("C�d. patron") => array('align'=>'center'),
...
if ($selected_id != -1) {
    ...
    //text_row(_("Patron:"), 'patron', null, 6, 6);
     label_row(_("Patr�n:"), $_POST['patron']); 
     hidden('patron', $_POST['patron']);
    //text_row(_("Tipo Cita:"), 'tipo_cita', null, 3, 3);	 
    //label_row(_("Tipo Cita:"), $_POST['tipo_cita']);
    //hidden('tipo_cita', $_POST['tipo_cita']); 
    ...
    type_patron_list_row(_("Patr�n:"), 'patron', null, true, true);
    //text_row(_("Tipo Cita:"), 'tipocita', null, 3, 3);
    tipo_cita_list_row(_("Tipo Cita:"), 'tipo_cita', null, true, false);
