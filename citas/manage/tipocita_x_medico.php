...
***********************************************************************/
//28/06/2010
$page_security = "SA_MEDICAL_APPOINMENT_TYPEAPPOINTMED";
$path_to_root="../..";
...
page(_("Type Appointment by medico"), false, false, "", $js);
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
  ...
  if (strlen($_POST['codmed']) == 0) 
	...
	if ($input_error !=1)
	{
    	if ($selected_id != -1) 
    	{
		  update_item_tpcitaxmedico($selected_id, $_POST['tipocita'], $_POST['precio']); 		
			display_notification(_('El item ha sido actualizado: '));
    	} 
    	else 
    	{
		    add_item_tpcitaxmedico($_POST['codmed'], $_POST['tipocita'], $_POST['precio']);                         
...
function obt_tipocita($row){
	  return get_tipocita($row['tipocita']);
}
...
if (get_post('medico_sel') || get_post('tipo_cita_sel')) {
	$table->set_sql($sql);
