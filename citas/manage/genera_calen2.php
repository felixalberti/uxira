...
$query_rs_medico = "SELECT * FROM ".TB_PREF."professional";
//$rs_medico = mysql_query($query_rs_medico, $conexion2) or die(mysql_error());
$rs_medico = db_query($query_rs_medico, "Can't retrieve record 0_professional");
...
function numerodia($dia){
    $miercoles = utf8_decode('Miércoles');
    $sabado = utf8_decode('Sábado');
	$semanaArray = array(
		"Domingo" => "1", 
		"Lunes" => "2", 
		"Martes" => "3", 
...
function dias_x_semana($colname_dia,$medico) {
	...
	$query_rs_semana = sprintf("SELECT * FROM ".TB_PREF."cm_diatrabxmedico ds, ".TB_PREF."cm_tabla_dias td where ds.enuso = 1 and td.nombre = %s and ds.codmed = %s and td.codigo = ds.coddia", "'".$colname_dia."'","'".$medico."'");
        display_notification('dias_x_semana -> '.$query_rs_semana);
...
function dias_x_medico($medico) {
	...
	display_notification('query dias x semana: '.$query_rs_semana);
	//$rs_semana = mysql_query($query_rs_semana, $conexion3) or die(mysql_error());
        $rs_semana = db_query($query_rs_semana, "Can't retrieve records 0_cm_diatrabxmedico");
	//$row_rs_semana = mysql_fetch_assoc($rs_semana);
        $row_rs_semana = db_fetch_assoc($rs_semana);
	display_notification(' Cant: '.$row_rs_semana['cant'].' ');
...
function generar_num($afecha, $codmed, $tipo_cita, $enuso) {
...
//echo $diasemana;
display_notification (" (".$dia.$mes.$ano.")");
...
display_notification("Select: ".$query_rs_numeros);
//mysql_select_db($database_conexion2, $conexion2);
...
display_notification("Registros: ".$totalRows_rs_numeros);
...
function dia_semana ($dia, $mes, $ano) {
    ...
    return utf8_decode($dias[date("w", mktime(0, 0, 0, $mes, $dia, $ano))]);
}
...
//if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
	...
	//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	$codmed = $_POST['medico'];
	display_notification('Médico: '.$codmed);
	$query_rs_cita = "SELECT * FROM ".TB_PREF."cm_citas WHERE fecha_cita >= '".date("Y/m/d")."' and codmed like '".$codmed."'";
        if (isset($_POST["generarfecha"]))
            display_notification($query_rs_cita.' Generar Fechas: '.$_POST["generarfecha"]);
        else
            display_notification($query_rs_cita);
	...
	$bdiastrabajo = dias_x_medico($codmed);	
	if(!($bdiastrabajo)){
	  $error = "<br>El(Los) médico(s) no tiene(n) dáas de trabajo activos";
          display_notification("El(Los) médico(s) no tiene(n) días de trabajo activos");
  }
	else
	display_notification("El(Los) médico(s) tiene(n) días de trabajo activos ");
        
        display_notification('Errores '.$error );
	...
	if  (isset($_POST["generarfecha"]) && ($_POST['generarfecha'] == "1" && $error == "" && $bdiastrabajo)) {
		...
		display_notification($deleteSQL);
		...
		for ($mes = 4; $mes <= 11; $mes++) {
			...
			for ($dia = 1; $dia <= 30; $dia++) {
			  ...
			  if ($fechacomp >= date("Ymd")) {			  
				  ...
				  $query_rs_medico = "SELECT * FROM ".TB_PREF."professional where professional_id like '".$codmed."'";
				  //$rs_medico = mysql_query($query_rs_medico, $conexion2) or die(mysql_error());
                                  $rs_medico = db_query($query_rs_medico, "Can't retrieve records into 0_professional");
				  //$row_rs_medico = mysql_fetch_assoc($rs_medico);
                                  $row_rs_medico = db_fetch_assoc($rs_medico);
				  //$cant = mysql_num_rows($rs_medico);
                                  $cant = db_num_rows($rs_medico);
				  //echo "aja";//,settype($cant,'string');
				  display_notification('dia:'.$dia.' - mes:'.$mes);
				  do { 
						  $codmedaux = $row_rs_medico['professional_id'];
						  if (dias_x_semana($diauso,$codmedaux))
							   {$enuso = 1; }
						  else
							   {$enuso = 0;
						  continue;}					  
						  display_notification( $codmed."-".$codmedaux );
						  ...
						  //
						  //Hacer ciclo y leer tablas que guarda los n�meros
						  display_notification( "Grabar".$codmed."-".$codmedaux." ".$fecha );	 
						  generar_num($fecha, $codmedaux, $_POST['tipocita'], $enuso);
		...
		for ($dia = 1; $dia <= $dialim; $dia++) {
			  ...
			  if ($fechacomp >= date("Ymd")) {
				  ...
				  $query_rs_medico = "SELECT * FROM ".TB_PREF."professional where professional_id like '".$codmed."'";
				  //$rs_medico = mysql_query($query_rs_medico, $conexion2) or die(mysql_error());
                                  $rs_medico = db_query($query_rs_medico, "Can't retrieve records into 0_cm_professional");
				  //$row_rs_medico = mysql_fetch_assoc($rs_medico);
                                  $row_rs_medico = db_fetch_assoc($rs_medico);
