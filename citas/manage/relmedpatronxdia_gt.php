...
page(_("Relación médico y patrón por día gestión total"), false, false, "", $js);
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
  ...
  if (strlen($_POST['codmed']) == 0) 
	...
	{
		$input_error = 1;
		display_error(_("El día no puede estar vacio."));
		set_focus('tipocita');
	}
...
$cols = array(
	...
	_("Día") => Array('align'=>'center','fun'=>'dia_semana'),
	_("Cód. patrón") => array('align'=>'center'),	
  _("Desc. patrón") => array('align'=>'left'),	
...
if ($selected_id != -1) {
    ...
    //tipo_cita_list_row(_("Día:"), 'coddia', null, false, false);
    day_week_list_row(_("Día:"), 'coddia', null,(!isset($_POST['coddia']) || $selected_id));
    //text_row(_("Día:"), 'coddia', null, 2, 1);
   
}
//text_row(_("patron:"), 'patron', null, 20, 20);
type_patron_list_row(_("Seleccionar un patrón:"), 'patron', null, false, true);
   ...
   if (list_updated('patron')) 
    {
	      ...
	      //
				...
				start_table($table_style);
						...
						$th = array( _("Tipo de cita"), _("Número"),
...
function get_res_numhor_x_patron($patron)
{
  $sql = "SELECT st.descripcion, nc.numero, nc.hora_cita, nc.codunidfunc  FROM ".TB_PREF.
   "cm_numeroscita nc left join ".TB_PREF."cm_system_tables st on st.codigotabla = 'TPCITA' and st.codigotipo = nc.tipo_cita".
   ", ".TB_PREF."cm_patron p where nc.patron = p.patron and nc.patron = '$patron'";
  return db_query($sql,"Las Horas y Tipo de cita x patrón no puedo ser retornado");
}
