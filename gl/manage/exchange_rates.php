...
add_js_file('calc_exchange_rate.js');
...
function handle_submit()
{
	...
	if ($selected_id != "")
	{
		...
		input_num('BuyRate'), input_num('BuyRate'), input_num('amount_provider'));
	}
	else
	{
...

{
	...
	if ($selected_id != "")
	{
		...
		$_POST['BuyRate'] = maxprec_format($myrow["rate_buy"]);
                ...
                $_POST['amount_provider'] = price_format($myrow["rate_current_day"]);
	...
	if (isset($_POST['get_rate']))
	{
        ...
        $ex_dec = $_SESSION["wa_current_user"]->prefs->exrate_dec();
        /*amount_row(_("Exchange Rate:"), 'BuyRate', null, '',
	  	submit('get_rate',_("Get"), false, _('Get current rate from') . ' ' . $xchg_rate_provider , true), 'max');*/
        echo "<tr>";
        new_amount_cells(_("*Tasa del Dia (BS): Coloque tasa y presione enter"), "amount_provider",  null, null, null, null, null, 'amount_provider');        
	echo "</tr>\n";        
	/*amount_row(_("Exchange Rate:"), 'BuyRate', null, '',
	  	submit('get_rate',_("Get"), false, _('Get current rate from') . ' ' . $xchg_rate_provider , true), $ex_dec);*/
	...
	end_table(1);
        if ($_SESSION['wa_current_user']->change_ex_rate == 1)
	submit_add_or_update_center($selected_id == '', '', 'both');
        else
        display_error ('You are not privileges');
...
/*$Ajax->addDisable(true, 'BuyRate', 1);
