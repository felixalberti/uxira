<?php
/**********************************************************************
    Copyright (C) may 2019 Uxira, C.A.
...
include_once($path_to_root . "/admission/includes/db/responsible_db.inc");
...
function create_cart($type, $trans_no)
{
	...
	if ($trans_no) {
		...
		elseif ($bank_trans["person_type_id"] == PT_QUICKENTRY)
			$_POST['person_id'] = $bank_trans["person_id"];
                elseif ($bank_trans["person_type_id"] == PT_RESP_PAYMENT)
		{
			$trans = get_customer_trans_deposit($trans_no, $type);	
			$_POST['person_id'] = $trans["person_id"];
			//$_POST['PersonDetailID'] = $trans["branch_code"];
                        $_POST['date_trans_bank_'] = sql2date($trans["date_trans_bank"]);
                        $_POST['ref_trans_bank'] = $trans["ref_trans_bank"];
		}
		else 
			$_POST['person_id'] = $bank_trans["person_id"];
...
function check_trans()
{
	...
	$input_error = 0;
        ...
        if (!isset($_POST['bank_account']) || (isset($_POST['bank_account']) && $_POST['bank_account'] == null)){
           display_error(_("Select a bank account"));
	...
	if (isset($_POST['settled_amount']) && in_array(get_post('PayType'), array(PT_SUPPLIER, PT_CUSTOMER)) && (input_num('settled_amount') <= 0)) {
        ...
        /*if ($_POST['PayType'] != PT_RESP_PAYMENT && isset($_POST['accountperpay']) && $_POST['accountperpay']== null){
            display_error(_("Seleccione una cuenta."));
	    set_focus('accountperpay');
	    $input_error = 1;
        }*/
        
        if (!is_date($_POST['date_trans_bank_']))
	{
		display_error(_("The entered date for date trans bank."));
		set_focus('date_trans_bank_');
		$input_error = 1;
	}
        
        if (isset($_POST['ref_trans_bank']) && $_POST['ref_trans_bank']== null)
	{
		display_error( _("You must enter a ref trans bank."));
		set_focus('ref_trans_bank');
		$input_error = 1;
	}
        
        /*if ($_POST['PayType'] == PT_RESP_PAYMENT){
            
            if (!get_post('person_id')) {
		display_error(_("You have to select customer."));
		//set_focus('person_id');
		//$input_error = 1;
            }
            else {
                $customer = get_post('person_id');
                $trans_items = get_allocatable_to_cust_transactions_uxira_fact_dep($customer, null, null);                            

                if (db_num_rows($trans_items) == 0) {
                    display_error ('Este responsable de pago no aparece como responsable en ninguna factura, revise (Problema Capa8)');
                    set_focus('person_id');
                    $input_error = 1;
                }
            }
...
if (isset($_POST['Process']) && !check_trans())
{
	...
	$new = $_SESSION['pay_items']->order_id == 0;
        ...
        if (!$new){
            $sql = "UPDATE ".TB_PREF."bank_trans SET person_id = ". db_escape($_POST['person_id']);
            $sql .= " WHERE type=".ST_BANKDEPOSIT.' and trans_no='. $_SESSION['pay_items']->order_id;      
            $err_msg = "The bank transaction could not be updated";
            db_query($sql, $err_msg);
            //
            $sql = "UPDATE ".TB_PREF."gl_trans SET person_id = ". db_escape($_POST['person_id']);
            $sql .= " WHERE type=".ST_BANKDEPOSIT.' and type_no='. $_SESSION['pay_items']->order_id;      
            $err_msg = "The bank transaction could not be updated";
            db_query($sql, $err_msg);
        ...
        $trans = write_bank_transaction_tmp(
		$_SESSION['pay_items']->trans_type, $_SESSION['pay_items']->order_id, $_POST['bank_account'],
		$_SESSION['pay_items'], $_POST['date_'],
		$_POST['PayType'], $_POST['person_id'], get_post('PersonDetailID'),
		$_POST['ref'], $_POST['memo_'], true, input_num('settled_amount'), $_POST['date_trans_bank_'], $_POST['ref_trans_bank']);
        if ($new){
	...
	new_doc_date($_POST['date_']);
        ...
        //Begin Felix Alberti 04/02/2017
        if (isset($_POST['accountperpay']) && $_POST['accountperpay']!=''){
            $balance = 0;
            $discount_tot = 0;
            $amount_tot = $_SESSION['pay_items']->gl_items_total();
            //display_notification('amount_tot1: '.$amount_tot);
            if ($amount_tot < 0) $amount_tot = $amount_tot * -1;
            //display_notification('amount_tot2: '.$amount_tot);
            $myrow = get_particular($_POST['accountperpay'], $_POST['person_id']);
            if (isset($myrow['id_account']))
            //display_notification('resta: '.$myrow['balance'].' - '.$amount_tot);
            if ($myrow['balance'] > 0)
            $balance = $myrow['balance']-$amount_tot;
            else
            $balance = $myrow['amount']-($myrow['paid'] + $amount_tot);
            //display_notification('balance: '.$balance);
            add_payer_trans_detail($_POST['accountperpay'], $_POST['person_id'], $amount_tot, $balance, $discount_tot, $trans[1], $_SESSION['pay_items']->trans_type); 
            
            if (!isset($myrow['id_account'])){
                //display_notification('proceso1');
                $compromiso = get_account_data_amount($_POST['accountperpay'],$_POST['person_id']);
                $balance = 0;
                $dif_a_pagar = 0;
                if ($compromiso){
                    if ($compromiso['balance_account'] >= $compromiso['balance_order']){
                        //display_notification('a');
                        $balance =   $compromiso['balance_account']-$amount_tot;
                        $dif_a_pagar = $compromiso['balance_account'];}
                    else {
                        //display_notification('b');
                        $balance =   $compromiso['balance_order']-$amount_tot;
                        $dif_a_pagar = $compromiso['balance_order'];
                    }
                    $paid = $compromiso['paid']+$amount_tot;                   
                }
                add_payer_trans($_POST['accountperpay'], $_POST['person_id'], $dif_a_pagar, $paid, $balance, 0);
           
            }
            else {
               //display_notification('proceso2: update_payer_trans '.$amount_tot.', '.$balance);
               update_payer_trans($_POST['accountperpay'], $_POST['person_id'], $amount_tot, $balance);
            }

            update_patient_account_x_pay($_POST['accountperpay'],$amount_tot);
            
            $payment_no = $trans[1];   
            $type = $trans[0];
                    
            add_cust_allocation($amount_tot, $type, $payment_no,
 			     		ST_PATIENT_ACCOUNT, $_POST['accountperpay'], 
                                        null, $_POST['accountperpay'], $_POST['ref'] );
            
            update_debtor_trans_allocation($type, $payment_no);
            
            $sql = "UPDATE `".TB_PREF."debtor_trans` trans			
		SET trans.account_associated=".db_escape($_POST['accountperpay']).
		" WHERE trans.type=".db_escape($type)." AND trans_no=".db_escape($payment_no);
            db_query($sql, "The debtor transaction record could not be modified for the allocation against it");
            
        }
        //End Felix Alberti 04/02/2017      
...
if ($_POST['PayType'] == PT_RESP_PAYMENT && !isset($_GET['ModifyDeposit']) && !isset($_POST['ModifyDeposit'])){
...
hidden('ModifyDeposit', $_GET['ModifyDeposit']);
