...
else
{
	...
	end_table(1);        
        ...
        $result = get_audit_trail_all(ST_BANKDEPOSIT,$trans_no);
        
        $trans_voided =db_fetch($result);
        
        if (db_num_rows ($result) > 0){
            display_note(_("Auditoria"));
            $result = get_audit_trail_all(ST_BANKDEPOSIT,$trans_no);
            $k = 0; //row colour counter
            start_table(TABLESTYLE, "width=80%"); 
            $th = array( _("Date"), _("Memo"), _("User"), _("ip"));
            table_header($th);
            while ($trans_voided =db_fetch($result)) {
                alt_table_row_color($k);

                       label_cell($trans_voided["gl_date"]);            
                       label_cell($trans_voided["description"]);
                       $myrow_user = get_user($trans_voided["user"]);
                       label_cell($myrow_user["user_id"]);
                       label_cell($trans_voided["ip"]);
