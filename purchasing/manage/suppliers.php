...
function supplier_settings(&$supplier_id)
{
	...
	if ($supplier_id) 
	{
		...
		$_POST['supp_address']  = $myrow["supp_address"];
                $_POST['type_person']  = $myrow["type_person"];//25/09/2018
	...
	text_row(_("Supplier Short Name:"), 'supp_ref', null, 30, 30);
        ...
        type_person_list_row(_("Type Person:"), 'type_person', null, false, false);//FAP 25/09/2018
	...
	//tax_groups_list_row(_("Tax Group:"), 'tax_group_id', null);
        tax_groups_list_row(_("Tax Group:"), 'tax_group_id', null, false, false, true);
	...
	//
        /*Begin Felix Alberti 28/08/2018*/
...
if (isset($_POST['submit'])) 
{
	...
	if (strlen($_POST['supp_ref']) == 0 || $_POST['supp_ref'] == "") 
	{
        ...
        if (strlen($_POST['gst_no']) == 0) 
	{
		display_error(_("GSTNo: ")._(" cannot be empty."));
		set_focus('gst_no');
		return false;
	}		
	elseif (!is_numeric($_POST['gst_no'])) 
	{
		display_error(_("GSTNo: ")._(" must be numeric."));
		set_focus('gst_no');
		return false;		
	}	
	...
	if ($input_error !=1 )
	{
		...
		if ($supplier_id) 
		{
			update_supplier($_POST['supplier_id'], $_POST['supp_name'], $_POST['supp_ref'], $_POST['address'],
				...
				$_POST['notes'], $_POST['tax_group_id'], get_post('tax_included', 0), $_POST['type_person']);
		...
		else 
		{
			add_supplier($_POST['supp_name'], $_POST['supp_ref'], $_POST['address'], $_POST['supp_address'],
				...
				$_POST['payment_discount_account'], $_POST['notes'], $_POST['tax_group_id'], check_value('tax_included'), $_POST['type_person']);
