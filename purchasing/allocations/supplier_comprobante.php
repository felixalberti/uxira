<?php
/**********************************************************************
    Copyright (C) FrontAccounting, LLC.
	Released under the terms of the GNU General Public License, GPL, 
	as published by the Free Software Foundation, either version 3 
	of the License, or (at your option) any later version.
    This program is distributed in the hope that it will be useful,
    but WITHOUT ANY WARRANTY; without even the implied warranty of
    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  
    See the License here <http://www.gnu.org/licenses/gpl-3.0.html>.
***********************************************************************/
$page_security = 'SA_SUPPLIERALLOC';
$path_to_root = "../..";

include($path_to_root . "/includes/ui/allocation_comprobante_cart.inc");
include_once($path_to_root . "/includes/session.inc");
include_once($path_to_root . "/includes/date_functions.inc");
include_once($path_to_root . "/includes/ui.inc");
include_once($path_to_root . "/includes/banking.inc");
include_once($path_to_root . "/sales/includes/sales_db.inc");
include_once($path_to_root . "/includes/db/sys_types_comprobantes_db.inc");
//include_once($path_to_root . "/purchasing/includes/ui/supp_alloc_ui.inc");

$js = "";
if ($use_popup_windows)
	$js .= get_js_open_window(900, 500);

add_js_file('allocate.js');

page(_($help_context = "Comprobante"), false, false, "", $js);

//--------------------------------------------------------------------------------

function clear_allocations()
{   //display_notification('clear');
	if (isset($_SESSION['alloc']))
	{
		unset($_SESSION['alloc']->allocs);
		unset($_SESSION['alloc']);
	}
	//session_register("alloc");
}
//--------------------------------------------------------------------------------

function edit_allocations_for_transaction($type, $trans_no)
{   //display_notification('edit_allocations_for_transaction');
	global $systypes_array;

	start_form();

	$cart = $_SESSION['alloc'];
    div_start('alloc_tbl_0');
    //display_heading(_("Allocation of") . " " . $systypes_array[$cart->type] . " # " . $cart->trans_no);

	display_heading($cart->person_name);

    //display_heading2(_("Date:") . " <b>" . $cart->date_ . "</b>");

   	//display_heading2(_("Total:"). " <b>" . price_format(-$cart->bank_amount).' '.$cart->currency."</b>");
	
	display_heading2(_("Type Taxe Retencion: Comprobante de Retencion "). " <b>" . $cart->tax_tyme_name."</b>");

	if ($cart->currency != $cart->person_curr)
	{
	    /*$total = _("Total in clearing currency:") . " <b>" . price_format(-$cart->amount)."</b>"
	    	. sprintf(" %s (%s %s/%s)", $cart->person_curr, exrate_format($cart->bank_amount/$cart->amount), $cart->currency, $cart->person_curr);
    	display_heading2($total);*/
	}
	div_end();
    echo "<br>";
    //display_notification('edit_allocations_for_transaction '.count($cart->allocs));
  	div_start('alloc_tbl');
    if (count($cart->allocs) > 0)
    {
		show_allocatable(true);

     	submit_center_first('UpdateDisplay', _("Refresh"), _('Start again allocation of selected amount'), true);
       	submit('Process', _("Process"), true, _('Process allocations'), 'default');
   		submit_center_last('Cancel', _("Back to Allocations"),
			_('Abandon allocations and return to selection of allocatable amounts'), 'cancel');
	}
	else
	{
    	display_note(_("There are no unsettled transactions to allocate."), 0, 1);
   		submit_center('Cancel', _("Back to Allocations"), true,
			_('Abandon allocations and return to selection of allocatable amounts'), 'cancel');
    }

	div_end();
	end_form();
}
//--------------------------------------------------------------------------------

if (isset($_POST['Process']))
{
	if (check_allocations())
	{
		$_SESSION['alloc']->write();
		clear_allocations();
		//$_POST['Cancel'] = 1;
		display_notification('programar-789');
	}
}

//--------------------------------------------------------------------------------

if (isset($_POST['Cancel']))
{
	clear_allocations();
	meta_forward($path_to_root . "/purchasing/allocations/supplier_allocation_main.php");
}

//--------------------------------------------------------------------------------

start_form();

if (!isset($_POST['supplier_id']))
	$_POST['supplier_id'] = get_global_supplier();

start_table(TABLESTYLE_NOBORDER);
start_row();

supplier_list_cells(_("Select a supplier: "), 'supplier_id', $_POST['supplier_id'], true);

date_cells(_("From:"), 'TransAfterDate', '', null, -30);
date_cells(_("To:"), 'TransToDate', '', null, 1);

tax_types_list_cells(_("Tax: "), 'tax_ret_sel', null, false, false);

submit_cells('RefreshInquiry', _("Search"),'',_('Refresh Inquiry'), 'default');



set_global_supplier($_POST['supplier_id']);

end_row();
end_table();

if (isset($_SESSION['alloc']) &&  get_post('RefreshInquiry'))
clear_allocations();

if (isset($_GET['trans_no']) && isset($_GET['trans_type']))
{   //display_notification(1);
	$_SESSION['alloc'] = new allocation($_GET['trans_type'], $_GET['trans_no'], @$_GET['supplier_id'], PT_SUPPLIER);
}

if (isset($_POST['supplier_id']))
{   //display_notification(2);
	$_SESSION['alloc'] = new allocation(ST_SUPPINVOICE, 0, $_POST['supplier_id'], PT_SUPPLIER, false, 0, $_POST['tax_ret_sel']);
}


if (get_post('UpdateDisplay'))
{
	//$_SESSION['alloc']->read();
	$Ajax->activate('alloc_tbl_0');
	$Ajax->activate('alloc_tbl');
}

if (isset($_SESSION['alloc']) ||  get_post('RefreshInquiry'))
{
	edit_allocations_for_transaction($_SESSION['alloc']->type, $_SESSION['alloc']->trans_no);
	$Ajax->activate('alloc_tbl_0');
	$Ajax->activate('alloc_tbl');
}

//--------------------------------------------------------------------------------

end_page();

?>