...
/*Begin Felix Alberti 16/05/2017*/
$awesome_menu_array = array (
	MENU_ENTRY => "fa fa-bars fa_custom",
	MENU_TRANSACTION => "fa fa-tasks fa_custom",
	MENU_INQUIRY => "fa fa-search fa_custom",
	MENU_REPORT => "fa fa-file-o fa_custom",
        MENU_MAINTENANCE => "fa fa-wrench fa_custom",
        MENU_UPDATE => "fa fa-refresh fa_custom",
        MENU_SETTINGS => "fa fa-cog fa_custom",
        MENU_SYSTEM => "fa fa-desktop fa_custom"
);
global $awesome_menu_array;
/*End Felix Alberti 16/05/2017*/
	...
	class app_function 
	{
		...
		var $access;
                var $category;
                var $count_notifications;
                var $type_notification;
		
		function app_function($label,$link,$access='SA_OPEN',$category='',$count_notifications=0,$type_notification='')
		{
			$this->label = $label;
			$this->link = $link;
			$this->access = $access;
	...
	class application 
	{
		...
		function add_lapp_function($level, $label,$link="",$access='SA_OPEN',$category='',$count_notifications=0,$type_notification='')
		{
			$this->modules[$level]->lappfunctions[] = new app_function($label, $link, $access, $category, $count_notifications, $type_notification);
		}
		
		function add_rapp_function($level, $label,$link="",$access='SA_OPEN',$category='',$count_notifications=0,$type_notification='')
		{
			$this->modules[$level]->rappfunctions[] = new app_function($label, $link, $access, $category, $count_notifications, $type_notification);
		}
