...
class dimensions_app extends application
{
	function dimensions_app()
	{
		...
		$this->application("proj", T_gettext($this->help_context = "&Dimensions"), $dim);

		if ($dim > 0)
		{
			$this->add_module(T_gettext("Transactions"));
			$this->add_lapp_function(0, T_gettext("Dimension &Entry"),
				"dimensions/dimension_entry.php?", 'SA_DIMENSION', MENU_ENTRY);
			$this->add_lapp_function(0, T_gettext("&Outstanding Dimensions"),
			...
			$this->add_lapp_function(1, T_gettext("Dimension &Inquiry"),
				"dimensions/inquiry/search_dimensions.php?", 'SA_DIMTRANSVIEW', MENU_INQUIRY);

			$this->add_rapp_function(1, T_gettext("Dimension &Reports"),
				"reporting/reports_main.php?Class=4", 'SA_DIMENSIONREP', MENU_REPORT);
			
			$this->add_module(T_gettext("Maintenance"));
			$this->add_lapp_function(2, T_gettext("Dimension &Tags"),
				"admin/tags.php?type=dimension", 'SA_DIMTAGS', MENU_MAINTENANCE);
                        $this->add_lapp_function(2, T_gettext("Setting Dimension"),
				"dimensions/inquiry/setting_dimensions.php?type=dimension", 'SA_DIMTAGS', MENU_MAINTENANCE);
