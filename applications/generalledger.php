...
class general_ledger_app extends application
{
	...
	{
		$this->application("GL", T_gettext($this->help_context = "&Banking and General Ledger"));
		...
		$this->add_lapp_function(0, _("&Deposits"),
			"gl/gl_bank.php?NewDeposit=Yes", 'SA_DEPOSIT', MENU_TRANSACTION);
                $this->add_lapp_function(0, _("&Deposits Responsible Payment"),
			"gl/gl_bank_temp.php?NewDeposit=Yes", 'SA_DEPOSIT', MENU_TRANSACTION);
                $this->add_lapp_function(0, _("&Journal Inquiry Pend."),
			"gl/inquiry/journal_inquiry_asign_customer.php?", 'SA_GLANALYTIC', MENU_TRANSACTION);
                $this->add_lapp_function(0, _("&Allocate Customer Deposits"),
			"sales/allocations/customer_allocation_bank_dep.php", 'SA_SALESALLOC', MENU_TRANSACTION);
