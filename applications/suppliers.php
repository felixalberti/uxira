...
class suppliers_app extends application 
{
	...
	{
		$this->application("AP", T_gettext($this->help_context = "&Purchases"));

		$this->add_module(T_gettext("Transactions"));
		$this->add_lapp_function(0, T_gettext("Purchase &Order Entry"),
			"purchasing/po_entry_items.php?NewOrder=Yes", 'SA_PURCHASEORDER', MENU_TRANSACTION);
		$this->add_lapp_function(0, T_gettext("&Outstanding Purchase Orders Maintenance"),
			"purchasing/inquiry/po_search.php?", 'SA_GRN', MENU_TRANSACTION);
		$this->add_lapp_function(0, T_gettext("Direct &GRN"),
